...
Another one uses cut locus; it is sketched in %\cite{petrunin-2020,panov-petrunin}, and 
...
Then at one point of $\gamma$ (distinct from its base $p_0$) its osculating circle $\sigma$ %globally?
 supports $\gamma$ from inside.
\end{thm}

First let us show that the theorem follows from the lemma.

\parit{Proof of \ref{thm:moon-strong} modulo \ref{thm:moon}.}
%Since $\gamma$ has signed curvature at most 1, each osculating circle lying at the left of $\gamma$ has radius at least 1.
...
Denote by $\gamma_1$ an arc of $\gamma$ from $p_1$ to a first point $q_1$ on $\sigma_1$ and such that it doesn't contain $p_0$ in its interior. %This may account for a drawing
